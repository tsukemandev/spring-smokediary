	...
	public CommandLineRunner prodInit(ChallengeLevelRepository challengeLevelRepository) {
		return (args) -> {
			...
			if (challengeLevel0 == null) {
				challengeLevelRepository.save(ChallengeLevel.builder().levelNumber(0L).reward("Level 0").duration(0L).image("").build()); //0분
			}

			ChallengeLevel challengeLevel1 = challengeLevelRepository.findByLevelNumber(1L);
			if (challengeLevel1 == null) {
				challengeLevelRepository.save(ChallengeLevel.builder().levelNumber(1L).reward("Reward for Level 1").duration(10000L).image(currentDomain + "/image/levels/level1.png").title("레벨 1: 시작 단계").message("새로운 시작을 축하합니다! 금연의 첫걸음을 내딛는 것이 가장 중요합니다. 오늘 당신이 내린 결정이 건강한 미래로 이끌어줄 것입니다. 용기를 내어 첫 발을 내딛은 당신이 자랑스럽습니다").build());
			}
